...
for ifo in ifos:
    ...
    pyplot.plot(psd[ifo].sample_frequencies, psd[ifo], label=ifo)

pyplot.yscale('log')
pyplot.xscale('log')
pyplot.ylim(1e-47, 1e-41)
pyplot.xlim(20, 1024)
...
pyplot.legend()
pyplot.show()
pyplot.savefig('chisq_significance_gw170814_strain.png')
pyplot.clf()
...
for w, title in [(7, 'Wide View'), (.15, 'Close to GW170814')]:
    ...
    for ifo in ifos:
        pyplot.plot(snr[ifo].sample_times, abs(snr[ifo]), label=ifo)

    pyplot.legend()
    pyplot.title(title)
    pyplot.grid()
    pyplot.xlim(m.time - w, m.time + w)
    pyplot.ylim(0, 15)
    pyplot.xlabel('Time (s)')
    pyplot.ylabel('Signal-to-noise (SNR)')
    pyplot.show()
    pyplot.savefig(f'chisq_significance_generate_template_waveform_{title}.png')
    pyplot.clf()
...
for ifo in ifos:
    pyplot.plot(chisq[ifo].sample_times, chisq[ifo], label=ifo)
    
pyplot.legend()
pyplot.grid()
pyplot.xlim(m.time -0.15, m.time + 0.15)
pyplot.ylim(0, 5)
pyplot.ylabel('$chi^2_r$')
pyplot.show()

pyplot.savefig('chisq_significance_data_fitting_model.png')
pyplot.clf()
...
for w, title in [(7, 'Wide View'), (.15, 'Close to GW170814')]:
    pyplot.figure(figsize=[14, 4])
    for ifo in ifos:
        pyplot.plot(snr[ifo].sample_times, nsnr[ifo], label=ifo)
    ...
    pyplot.ylim(0, 15)
    pyplot.xlabel('Time (s)')
    pyplot.ylabel('Re-weighted Signal-to-noise')
    pyplot.show()
    
    pyplot.savefig(f'chisq_significance_rewrite_snr_{title}.png')
    pyplot.clf()
...
for ifo in ifos:
    ...
    if ifo != 'V1':
        ...
        pyplot.axvspan(ptime[ifo] - tof[ifo], ptime[ifo] + tof[ifo], alpha=0.2, lw=10)
...
pyplot.legend()
pyplot.grid()
pyplot.xlim(m.time - .05, m.time + .10)
pyplot.ylim(0, 15)
pyplot.xlabel('Time (s)')
pyplot.ylabel('Re-weighted Signal-to-noise')
pyplot.show()
...
pyplot.savefig(f'chisq_significance_calculating_background_and_significance.png')
pyplot.clf()
...
pyplot.scatter(peaks, pcurve, label='Off-source (Noise Background)', color='black')

pyplot.axvline(onsource, label='On-source', color='red')
pyplot.axhline(pvalue, color='red')
...
pyplot.xlabel('Re-weighted Signal-to-noise')
